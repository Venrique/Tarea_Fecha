...
// VERSIÓN MEJORADA QUE CONSIDERA MESES

function getAge(birthday) {
    var actual = new Date();
    var nacimiento = new Date(birthday);
    var edad = actual.getFullYear() - nacimiento.getFullYear();
    var m = actual.getMonth() - nacimiento.getMonth();
    if (m < 0 || (m === 0 && actual.getDate() < nacimiento.getDate())) {
        edad--;
    }
    return edad;
}
